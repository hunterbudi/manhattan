        ...
        if (isset($_POST['simpan'])) { // jika tombol editsubmit ditekan
            ...
            $edit = mysql_query("UPDATE kiriman SET
							...
							judul_en='$judul_en',
                                                        ...
                                                        isi_en='$isi_en',
                                                        urut='$urut'      
					WHERE id='$id' and jenis='pelintas_batas'");
            echo "<script>alert ('FAQ Pelintas Batas berhasil diperbaharui, terima kasih')</script>";
            echo "<script type='text/javascript'>window.location='?module=pelintas_batas'</script>";
        } else {
            $id = $_GET['id']; // menangkap id
            $sql = "SELECT * FROM kiriman WHERE id=$id and jenis='pelintas_batas'"; // memanggil data dengan id yang ditangkap tadi
            $query = mysql_query($sql);
            $data = mysql_fetch_array($query);
            ?>
            <form name="impor" method="post" action="?module=pelintas_batas_edit" enctype="multipart/form-data">  
