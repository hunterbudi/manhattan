<?php
$query = "SELECT * FROM tentang WHERE kode LIKE 'tupoksi'";
...
<aside class="right-side">
    ...
    <form name="tupoksi" method="post" action="?module=tupoksi">  
    ...
    if (isset($_POST['simpan'])){
        ...
        $query = "UPDATE tentang SET isi_id='$isi_id',isi_en='$isi_en' WHERE kode LIKE 'tupoksi'";
        $sql = mysql_query($query);
        
        if ($sql){        
                echo "<script>alert ('Konten berhasil diupdate, terima kasih')</script>";
		echo '<script type="text/javascript">window.location="?module=tupoksi"</script>'; 
            }
        }
    ?>
        <br>
