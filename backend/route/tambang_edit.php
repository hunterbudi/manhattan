...
<aside class="right-side">
    ...
    <section class="content-header">
        <h1>
            Fasilitas
            <small>Edit Pertambangan</small>
        </h1>
        <ol class="breadcrumb">
            <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
            <li class="active">Fasilitas</li>
            <li class="active">Edit Pertambangan</li>
        </ol>
    </section>
    <!-- Main content -->
    <section class="content">
        ...
        if (isset($_POST['simpan'])) { // jika tombol editsubmit ditekan
            ...
            $edit = mysql_query("UPDATE administrasi SET
							...
							judul_en='$judul_en',
                                                        ...
                                                        isi_en='$isi_en',
                                                        urut='$urut'
					WHERE id='$id' and jenis='tambang'");
            echo "<script>alert ('FAQ Fasilitas tentang Pertambangan berhasil diperbaharui, terima kasih')</script>";
            echo "<script type='text/javascript'>window.location='?module=tambang'</script>";
        } else {
            $id = $_GET['id']; // menangkap id
            $sql = "SELECT * FROM administrasi WHERE id=$id and jenis='tambang'"; // memanggil data dengan id yang ditangkap tadi
            $query = mysql_query($sql);
            $data = mysql_fetch_array($query);
            ?>
            <form name="fasilitas" method="post" action="?module=tambang_edit" enctype="multipart/form-data">  
        ...
        <tbody>
           ...
           ?>
            ...
            <span> <a href=?module=delete&pages=administrasi&id=<?php echo $id ?> title='delete' onClick="return confirm('FAQ Fasilitas tentang Pertambangan akan dihapus, Anda yakin?');"  class='btn btn-outline btn-danger btn-xs'> <i class='fa fa-trash-o'></i></a> </span>
            </td>
            </tr>
            <?php
            $no++;
