<aside class="right-side">
    ...
    <h1>
    Fasilitas
    <small>Rekam Pembebasan Bea Masuk</small>
    </h1>
    <ol class="breadcrumb">
        <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">Fasilitas</li>
        <li class="active">Rekam Pembebasan Bea Masuk</li>
    </ol>
    </section>
    <!-- Main content -->
    <section class="content">
    <script src="ckeditor/ckeditor.js"></script>
    <form name="fasilitas" method="post" action="?module=bebas_bm" enctype="multipart/form-data">  
      ...
      <div role="tabpanel" class="tab-pane fade" id="profile" aria-labelledBy="profile-tab">
          ...
          <table class="table">
                <tr>
                  <td width="20%"></td>
                  <td width="2%"></td>
                  <td width="78%"><input type="submit" class="btn btn-primary" value="SIMPAN" name="simpan" onClick="return confirm('FAQ Fasilitas tentang Pembebasan Bea Masuk akan disimpan?');"></td>
                </tr>        
          </table>
    </div>
    </div>
    ...
    if (isset($_POST['simpan'])){
        ...
        function newID() {
                    $now = date('Y');
                    $query = "SELECT max(urut) as maxID FROM administrasi where jenis='bebas_bm'";
                    $hasil = mysql_query($query);
                    $data = mysql_fetch_array($hasil);
                    $idMax = $data['maxID'];

                    $querybaru = "SELECT * FROM administrasi where urut='$idMax' and  jenis='bebas_bm'";
        ...
        $query = "INSERT INTO administrasi (judul_id,judul_en,isi_id,isi_en,jenis,urut)"
                . " VALUES ('$judul_id','$judul_en','$isi_id','$isi_en','bebas_bm','$nourutbaru')";
        $sql = mysql_query($query);
        
        if ($sql){
            echo "<script>alert ('Faq Fasilitas tentang Pembebasan Bea Masuk berhasil disimpan, terima kasih')</script>";
            echo '<script type="text/javascript">window.location="?module=bebas_bm"</script>'; 
        }
    }
    ?>
    <br><br>
    <table id="example1" class="table table-bordered table-striped">
        ...
        <tbody>
            ...
            $sql = mysql_query("SELECT * from administrasi where jenis='bebas_bm' ORDER BY id DESC");                               
            $no = 1;
           ...
           ?>
            ...
            <td class='odd gradeX'>
            <span> <a href=?module=bebas_bm_edit&id=<?php echo  $id ?> title='edit' class='btn btn-outline btn-primary btn-xs '> <i class='fa fa-edit'></i></a></span>
            <span> <a href=?module=delete&pages=administrasi&id=<?php echo $id ?> title='delete' onClick="return confirm('FAQ Fasilitas Pembebasan BM akan dihapus, Anda yakin?');"  class='btn btn-outline btn-danger btn-xs'> <i class='fa fa-trash-o'></i></a> </span>
            </td>
            </tr>
            <?php
            $no++;
