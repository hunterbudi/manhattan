    ...
    <section class="content">
        ...
        if (isset($_POST['simpan'])) { // jika tombol editsubmit ditekan
            ...
            $edit = mysql_query("UPDATE kiriman SET
							...
							judul_en='$judul_en',
                                                        ...
                                                        isi_en='$isi_en',
                                                        urut='$urut'      
					WHERE id='$id' and jenis='kiriman_pos'");
            echo "<script>alert ('FAQ Kiriman Pos berhasil diperbaharui, terima kasih')</script>";
            echo "<script type='text/javascript'>window.location='?module=kiriman_pos'</script>";
        } else {
            $id = $_GET['id']; // menangkap id
            $sql = "SELECT * FROM kiriman WHERE id=$id and jenis='kiriman_pos'"; // memanggil data dengan id yang ditangkap tadi
            $query = mysql_query($sql);
            $data = mysql_fetch_array($query);
            ?>
            <form name="impor" method="post" action="?module=kiriman_pos_edit" enctype="multipart/form-data">  
                    ...
                    <div id="myTabContent" class="tab-content">
                        ...
                        <table class="table">
                            ...
                            <tr>
                                <td width="20%"></td>
                                <td width="2%"></td>
                                <td width="78%"><input type="submit" class="btn btn-primary" value="SIMPAN" name="simpan" onClick="return confirm('FAQ Kiriman Pos akan disimpan?');"></td>
                            </tr>        
                        </table>
                    </div>
                </div>
        ...
        <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                    ...
                    $sql = mysql_query("SELECT * from kiriman where jenis='kiriman_pos' ORDER BY urut DESC");
                    $no = 1;
                    while ($r = mysql_fetch_array($sql)) {
                      ...
                      ?>
                        <tr align='center'>
                            ...
                            <td class='odd gradeX'>
                                <span> <a href=?module=kiriman_pos_edit&id=<?php echo $id ?> title='edit' class='btn btn-outline btn-primary btn-xs '> <i class='fa fa-edit'></i></a></span>
                                <span> <a href=?module=delete&pages=kiriman&id=<?php echo $id ?> title='delete' onClick="return confirm('FAQ Kiriman Pos akan dihapus, Anda yakin?');"  class='btn btn-outline btn-danger btn-xs'> <i class='fa fa-trash-o'></i></a> </span>
                            </td>
                        </tr>
                        <?php
                        $no++;
