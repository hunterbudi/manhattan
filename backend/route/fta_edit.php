...
<aside class="right-side">
    ...
    <section class="content-header">
        <h1>
            Administrasi
            <small>Edit Fta</small>
        </h1>
        <ol class="breadcrumb">
            <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
            <li class="active">Administrasi</li>
            <li class="active">Edit Fta</li>
        </ol>
    </section>
    <!-- Main content -->
        ...
        if (isset($_POST['simpan'])) { // jika tombol editsubmit ditekan
            ...
            $edit = mysql_query("UPDATE administrasi SET
							...
							judul_en='$judul_en',
                                                        ...
                                                        isi_en='$isi_en',
                                                        urut='$urut'
					WHERE id='$id' and jenis='fta'");
            echo "<script>alert ('FAQ Fta berhasil diperbaharui, terima kasih')</script>";
            echo "<script type='text/javascript'>window.location='?module=fta'</script>";
        } else {
            $id = $_GET['id']; // menangkap id
            $sql = "SELECT * FROM administrasi WHERE id=$id and jenis='fta'"; // memanggil data dengan id yang ditangkap tadi
            $query = mysql_query($sql);
            $data = mysql_fetch_array($query);
            ?>
            <form name="fta" method="post" action="?module=fta_edit" enctype="multipart/form-data">  
