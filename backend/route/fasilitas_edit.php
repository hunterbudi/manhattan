        ...
        if (isset($_POST['simpan'])) { // jika tombol editsubmit ditekan
            ...
            $edit = mysql_query("UPDATE administrasi SET
							...
							judul_en='$judul_en',
                                                        ...
                                                        isi_en='$isi_en',
                                                        urut='$urut'
					WHERE id='$id' and jenis='fasilitas'");
            echo "<script>alert ('FAQ Fasilitas berhasil diperbaharui, terima kasih')</script>";
            echo "<script type='text/javascript'>window.location='?module=fasilitas'</script>";
        } else {
            $id = $_GET['id']; // menangkap id
            $sql = "SELECT * FROM administrasi WHERE id=$id and jenis='fasilitas'"; // memanggil data dengan id yang ditangkap tadi
            $query = mysql_query($sql);
            $data = mysql_fetch_array($query);
            ?>
            <form name="fasilitas" method="post" action="?module=fasilitas_edit" enctype="multipart/form-data">  
